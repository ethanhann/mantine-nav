...
import { NavShell, NavSidebar, NavHeader, NavGroup, useRemoteNavItems } from '../../src';
import type { RemoteNavItem, NavItemResolvers } from '../../src';
...
const MOCK_API_RESPONSE: RemoteNavItem[] = [
  { id: 'home', type: 'link', label: 'Home', href: '/', icon: 'home', weight: -10 },
  { id: 'section-commerce', type: 'section', label: 'Commerce' },
  {
    ...
    defaultOpened: true,
    children: [
      { id: 'catalog', type: 'link', label: 'Catalog', href: '/products/catalog' },
      { id: 'inventory', type: 'link', label: 'Inventory', href: '/products/inventory', badge: 'new' },
    ],
  },
  { id: 'orders', type: 'link', label: 'Orders', href: '/orders', icon: 'orders', weight: 20 },
  { id: 'customers', type: 'link', label: 'Customers', href: '/customers', icon: 'users', weight: 30 },
  { id: 'div-1', type: 'divider' },
  { id: 'section-insights', type: 'section', label: 'Insights' },
  { id: 'analytics', type: 'link', label: 'Analytics', href: '/analytics', icon: 'chart', weight: 40 },
  { id: 'div-2', type: 'divider' },
  { id: 'docs', type: 'link', label: 'Documentation', href: 'https://mantine.dev', icon: 'external', external: true },
  { id: 'feedback', type: 'link', label: 'Send Feedback', href: '#', icon: 'message' },
  { id: 'settings', type: 'link', label: 'Settings', href: '/settings', icon: 'settings', weight: 100 },
];
...
const resolvers: NavItemResolvers = {
  ...
  badges: {
    new: <Badge size="xs" variant="light" color="teal">New</Badge>,
  },
  onClick: {
...
function RemoteNavDemo() {
  ...
  return (
    <NavShell
      header={<NavHeader logo={<Text fw={700} size="lg">Remote Nav</Text>} />}
      sidebar={
        <NavSidebar>
          {isLoading ? (
            <Center py="xl">
    ...
    >
      <Text size="xl" fw={700} mb="md">API-Driven Navigation</Text>
      <Text c="dimmed" mb="sm">
        The sidebar items are loaded from a simulated API response after a 1.5s delay.
        A resolver map on the client hydrates icon/badge string keys into React components.
      </Text>
...
function InstantDataDemo() {
  ...
  return (
    <NavShell
      header={<NavHeader logo={<Text fw={700} size="lg">Static JSON</Text>} />}
      sidebar={
        <NavSidebar>
          <NavGroup items={items} currentPath="/" />
        </NavSidebar>
    ...
    >
      <Text size="xl" fw={700} mb="md">Static JSON Data</Text>
      <Text c="dimmed">
        Same JSON payload passed synchronously — no loading state. Useful for SSR or cached responses.
      </Text>
    </NavShell>
  );
}
