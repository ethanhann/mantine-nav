...
describe("Spec 031: useStarredPages", () => {
	it("starts empty", () => {
		const { result } = renderHook(() =>
			useStarredPages({ storageKey: "test-starred" }),
		);
	...
	it("star and unstar work", () => {
		const { result } = renderHook(() =>
			useStarredPages({ storageKey: "test-star-ops" }),
		);
	...
	it("toggleStar works", () => {
		const { result } = renderHook(() =>
			useStarredPages({ storageKey: "test-star-toggle" }),
		);
	...
	it("reorder works", () => {
		const { result } = renderHook(() =>
			useStarredPages({ storageKey: "test-star-reorder" }),
		);
