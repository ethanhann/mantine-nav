...
import type { NavItemType, NavGroupItem, NavLinkItem } from '../types';
...
function convertNodes<TData>(
  ...
  for (const node of nodes.values()) {
    ...
    if (hasChildren) {
      ...
      const groupConfig = config ?? { type: 'group' as const, label: node.id.split('.').pop()! };
      result.push({
        ...groupConfig,
        id: node.id,
        type: 'group',
        label: (groupConfig as { label?: string }).label ?? node.id.split('.').pop()!,
...
export function useNavRegistry<TData = unknown>(): UseNavRegistryReturn<TData> {
  ...
  const register = useCallback((id: string, config: NavRegistryEntry<TData>) => {
